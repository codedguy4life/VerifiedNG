...
function canonicalCategory(value) {
  ...
  for (const [category, aliases] of Object.entries(serviceAliases)) {
    ...
    if (aliases.some((alias) => normalizeSearchText(alias) === text)) {
      return normalizedCategory;
    }
  }
...
function matchesLocation(provider, query) {
  ...
  if (!q || q === "nigeria" || q === "all locations" || q === "all") {
    return true;
  }
  ...
  return aliases.some((alias) => location.includes(normalizeSearchText(alias)));
}
  ...
  try {
    ...
    */
    ...
    const dbProviders = data.providers.map((p) => ({
      ...
      availText: getAvailabilityText(p.availability),
    ...
    console.log("Real providers loaded:", providers);
  } catch (error) {
...
function buildLocationFilter() {
  ...
  providers.forEach((provider) => {
    ...
    locationCounts[displayLocation] =
      (locationCounts[displayLocation] || 0) + 1;
  });
...
function updateFilterCounts(filteredProviders) {
  ...
  filteredProviders.forEach((provider) => {
    if (provider.category) {
      categoryCounts[provider.category] =
        (categoryCounts[provider.category] || 0) + 1;
    }
  ...
  document
    .querySelectorAll("#categoryFilter .filter-option")
    .forEach((label) => {
      const input = label.querySelector("input");

      const countElement = label.querySelector(".filter-count");
  ...
  document
    .querySelectorAll("#locationFilter .filter-option")
    .forEach((label) => {
      const input = label.querySelector("input");
      ...
      countElement.textContent = locationCounts[location] || 0;
    });
}
...
function matchesService(provider, service) {
  const query = normalizeSearchText(service);

  if (!query || query === "all services") {
    return true;
  }

  const canonical = canonicalCategory(service);

  const providerCategory = canonicalCategory(provider.category);

  if (providerCategory === canonical) {
    return true;
  }

  const providerText = normalizeSearchText(
    [
      provider.name,
      provider.category,
      provider.role,
  ...
  if (providerText.includes(query)) {
    return true;
  }
  ...
  return aliases.some((alias) =>
    providerText.includes(normalizeSearchText(alias)),
  );
}
...
function matchesAvailability(provider, requested) {
  if (!requested) {
    return true;
  }

  return (
    provider.availability === requested ||
    (requested === "available" && provider.availability === "online")
  );
}
...
function renderCards(list) {
  ...
  if (list.length === 0) {
    ...
    const heading = createTextElement("h3", null, "No providers found");
    ...
    noResults.appendChild(iconWrapper);
  ...
  list.forEach((p, i) => {
    ...
    card.addEventListener("click", () => {
      window.location.href = `all-providers-profile.html?id=${encodeURIComponent(p.id)}`;
    });
    ...
    const name = createTextElement("span", "card-name", p.name);
    ...
    if (p.verified) {
      ...
      verified.className = "verified-pill";

      const verifiedIcon = document.createElement("i");

      verifiedIcon.className = "bi bi-patch-check";
    ...
    const role = createTextElement("div", "card-role", p.role);
    ...
    const stars = createTextElement("span", "stars", "★★★★★");
    ...
    if (p.reviewCount > 0) {
      const reviews = createTextElement(
        "span",
        "reviews",
    ...
    cardLocation.appendChild(locationIcon);

    cardLocation.appendChild(
      document.createTextNode(` ${String(p.location ?? "")}`),
    );

    cardInfo.appendChild(nameRow);

    cardInfo.appendChild(role);
    ...
    (p.tags || []).forEach((tag) => {
      const tagElement = createTextElement("span", "tag", tag);
    ...
    jobsBox.className = "cs-box";

    const jobsValue = document.createElement("div");

    jobsValue.className = "cs-val";

    if (p.jobs > 0) {
      jobsValue.appendChild(document.createTextNode(String(p.jobs)));

      const sup = document.createElement("sup");

      sup.textContent = "+";

      jobsValue.appendChild(sup);
    } else {
      jobsValue.textContent = "—";
    }

    const jobsLabel = createTextElement("div", "cs-label", "Jobs Done");

    jobsBox.appendChild(jobsValue);

    jobsBox.appendChild(jobsLabel);

    // Experience

    const experienceBox = document.createElement("div");

    experienceBox.className = "cs-box";

    const experienceValue = createTextElement(
      "div",
      "cs-val",
      p.experienceYears,
    );

    const experienceLabel = createTextElement("div", "cs-label", "Experience");

    experienceBox.appendChild(experienceValue);

    experienceBox.appendChild(experienceLabel);

    // Rating

    const ratingBox = document.createElement("div");

    ratingBox.className = "cs-box";

    const ratingValue = createTextElement(
      "div",
      "cs-val",
      p.rating > 0 ? `${p.rating}★` : "New",
    );

    const ratingLabel = createTextElement("div", "cs-label", "Rating");

    ratingBox.appendChild(ratingValue);

    ratingBox.appendChild(ratingLabel);

    cardStats.appendChild(jobsBox);

    cardStats.appendChild(experienceBox);

    cardStats.appendChild(ratingBox);
    ...
    const from = createTextElement("div", "from", "Starting price");

    const amount = createTextElement("span", "amount", p.price);

    const per = createTextElement("span", "per", p.per);

    priceInfo.appendChild(from);

    priceInfo.appendChild(amount);

    priceInfo.appendChild(per);
    ...
    footerRight.style.flexDirection = "column";

    footerRight.style.alignItems = "flex-end";
    ...
    availabilityText.className = "avail-text";

    const availabilityDot = document.createElement("span");

    availabilityDot.className = `avail-dot ${p.availability}`;

    const availabilityLabel = createTextElement("span", null, p.availText);

    availabilityText.appendChild(availabilityDot);

    availabilityText.appendChild(availabilityLabel);
    ...
    messageButton.className = "btn-msg";
    ...
    messageIcon.className = "bi bi-chat-dots";

    messageButton.appendChild(messageIcon);

    messageButton.appendChild(document.createTextNode(" Message"));

    // Hire button

    const hireButton = document.createElement("button");

    hireButton.className = "btn-hire";
    ...
    cardActions.appendChild(hireButton);

    footerRight.appendChild(availabilityText);

    footerRight.appendChild(cardActions);
...
function applyFilters() {
  ...
  if (service && service !== "All Services") {
    filtered = filtered.filter((provider) => matchesService(provider, service));
  }
  ...
  if (checkedCategories.length) {
    filtered = filtered.filter((provider) =>
      checkedCategories.some(
        (category) =>
          canonicalCategory(provider.category) === canonicalCategory(category),
      ),
    );
  }
  ...
  if (checkedLocations.length) {
    filtered = filtered.filter((provider) =>
      checkedLocations.some((location) => matchesLocation(provider, location)),
    );
  }
  ...
  const checkedAvailability = [
    ...
    .filter(
      (input) =>
        ["available", "weekend", "24hr"].includes(input.value) && input.checked,
    )
    .map((input) => input.value);

  if (checkedAvailability.length) {
    filtered = filtered.filter((provider) =>
      checkedAvailability.some((availability) =>
        matchesAvailability(provider, availability),
      ),
    );
  }
...
function handleSearch() {
  const service =
    document.getElementById("navServiceInput")?.value.trim() || "";

  const location =
    document.getElementById("navLocationInput")?.value.trim() || "";
...
window.addEventListener("load", async function () {
  ...
  if (searchDisplay) {
    searchDisplay.textContent = `"${service}"`;
  }
  ...
  if (locationDisplay) {
    ...
    locationDisplay.appendChild(icon);

    locationDisplay.appendChild(document.createTextNode(` ${location}`));
  }
  ...
  if (serviceTag) {
    ...
    const serviceText = document.createTextNode(` ${service}`);
    ...
    removeButton.addEventListener("click", removeServiceFilter);

    serviceTag.appendChild(icon);
    ...
    serviceTag.style.display =
      service !== "All Services" ? "inline-flex" : "none";
  }
  ...
  if (locTag) {
    ...
    removeButton.addEventListener("click", removeLocationFilter);

    locTag.appendChild(icon);

    locTag.appendChild(locationText);

    locTag.appendChild(removeButton);
  ...
  if (serviceInput) {
    serviceInput.value = service !== "All Services" ? service : "";
  }

  if (locationInput) {
    locationInput.value = location !== "Nigeria" ? location : "";
  }
  ...
  if (navActions && user) {
    ...
    dashboardLink.appendChild(document.createTextNode(`Hi, ${firstName} `));

    const personIcon = document.createElement("i");

    personIcon.className = "bi bi-person-circle";

    dashboardLink.appendChild(personIcon);
